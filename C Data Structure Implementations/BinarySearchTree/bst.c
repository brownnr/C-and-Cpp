...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	/*base case - if node passed is null, make a new node*/
	if (cur == 0) {
	...
	/*otherwise recursively keep trying until above base case is found*/
	if (compare(val, cur->val) == -1) {
		...
		/*value is already in the tree so don't add a duplicate*/
		printf("Value was not added because it is already in the tree.\n");
	}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	/*make temp node point to root*/
	struct Node *curr;
	curr = tree->root;

	//if(compare(val, curr->val) == 0) {return 1;}

	/*while we haven't found the value*/
	while(compare(val, curr->val) != 0) {
		/*search left tree is the value is < the current node*/
		if(compare(val, curr->val) == -1) {
			if(curr->left == 0)
				/*not found*/
				return 0;
			else
		...
		/*search right tree is the value is > the current node*/
		else if(compare(val, curr->val) == 1) {
			if(curr->right == 0)
				/*not found*/
				return 0;
			else
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	/*otherwise recursive call on left child*/
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	/*if current node's value is equal to val*/
	if(compare(val, cur->val) == 0) {
		...
		/*otherwise change cur's value to leftmost child and call _removeLeftMost*/
		cur->val = _leftMost(cur->right);
		cur->right = _removeLeftMost(cur->right);
	}
	/*else if val is < cur's value*/
	else if(compare(val, cur->val) == -1) {
		cur->left = _removeNode(cur->left, val);
	}
	/*else it's greater than cur's value*/
	else {
